// CSV parsing and chart inference. Given the CSV files of a results session,
// work out which ones can be drawn and produce chart specs that the browser
// renders (assets/charts.js). Everything here is heuristic and defensive:
// a CSV that does not fit any rule simply gets a table view.
import path from 'node:path';
import { titleCase, median } from './util.mjs';

export function parseCSV(text) {
  const rows = [];
  let row = []; let field = ''; let inQ = false;
  const s = String(text || '');
  for (let i = 0; i < s.length; i++) {
    const c = s[i];
    if (inQ) {
      if (c === '"') { if (s[i + 1] === '"') { field += '"'; i++; } else inQ = false; }
      else field += c;
    } else if (c === '"') inQ = true;
    else if (c === ',') { row.push(field); field = ''; }
    else if (c === '\n' || c === '\r') {
      if (c === '\r' && s[i + 1] === '\n') i++;
      row.push(field); field = '';
      if (row.some((f) => f !== '')) rows.push(row);
      row = [];
    } else field += c;
  }
  row.push(field);
  if (row.some((f) => f !== '')) rows.push(row);
  if (!rows.length) return { header: [], rows: [] };
  const header = rows[0].map((h) => h.trim());
  return { header, rows: rows.slice(1).map((r) => header.map((_, i) => (r[i] ?? '').trim())) };
}

const NUM_RE = /^[-+]?(\d+\.?\d*|\.\d+)([eE][-+]?\d+)?$/;
const BOOL = new Set(['true', 'false', 'yes', 'no', 'pass', 'fail', 'passed', 'failed']);

export function analyzeColumns(header, rows) {
  return header.map((name, i) => {
    const raw = rows.map((r) => r[i] ?? '');
    const nonEmpty = raw.filter((v) => v !== '');
    const nums = nonEmpty.filter((v) => NUM_RE.test(v));
    const bools = nonEmpty.filter((v) => BOOL.has(v.toLowerCase()));
    const times = nonEmpty.filter((v) => /^\d{4}-\d{2}-\d{2}/.test(v) && !Number.isNaN(Date.parse(v)));
    let type = 'string';
    if (nonEmpty.length && nums.length / nonEmpty.length >= 0.9) type = nums.every((v) => /^[-+]?\d+$/.test(v)) ? 'int' : 'number';
    else if (nonEmpty.length && bools.length === nonEmpty.length) type = 'bool';
    else if (nonEmpty.length && times.length === nonEmpty.length) type = 'time';
    const unique = new Set(nonEmpty).size;
    let values;
    if (type === 'int' || type === 'number') values = raw.map((v) => (v === '' || !NUM_RE.test(v) ? null : Number(v)));
    else if (type === 'time') values = raw.map((v) => (v === '' ? null : Date.parse(v)));
    else values = raw;
    const monotonic = (type === 'int' || type === 'number' || type === 'time') && values.length > 1
      && values.every((v, k) => k === 0 || v == null || values[k - 1] == null || v > values[k - 1]);
    return { name, index: i, type, values, unique, constant: unique <= 1, monotonic };
  });
}

const X_NAMES = ['trial', 'step', 'epoch', 'iteration', 'iter', 'index', 'idx', 'i', 'n', 't', 'time', 'timestamp',
  'timestamp_utc', 'date', 'datetime', 'x', 'batch', 'tick', 'seq', 'sample_index', 'token', 'tokens', 'position', 'layer',
  'size', 'n_tokens', 'context', 'context_length', 'batch_size', 'threads'];
const EXCLUDE_Y = ['seed', 'id', 'run', 'run_id', 'pid', 'sample', 'image_digest', 'digest', 'hash', 'sha256'];
const PRIORITY = ['tflops', 'tokens', 'tok', 'throughput', 'latency', 'accuracy', 'loss', 'f1', 'auc', 'time', 'seconds', 'ms'];

const UNITS = [
  [/(^|_)(tflops?|tflop_s|tflop\/s)$/i, 'TFLOP/s'], [/(^|_)(gflops?)$/i, 'GFLOP/s'], [/(^|_)(tokens?_(per_)?s(ec)?|tok_s|tps|tokens_s)$/i, 'tokens/s'],
  [/(^|_)(ms|millis|milliseconds)$/i, 'ms'], [/(^|_)(us|micros|microseconds)$/i, 'µs'], [/(^|_)(s|sec|secs|seconds|elapsed)$/i, 's'],
  [/(^|_)(pct|percent|percentage)$/i, '%'], [/(^|_)(w|watts?)$/i, 'W'], [/(^|_)(c|celsius|temp_c)$/i, '°C'],
  [/(^|_)(kb)$/i, 'KB'], [/(^|_)(mb)$/i, 'MB'], [/(^|_)(gb)$/i, 'GB'], [/(^|_)(gib)$/i, 'GiB'], [/(^|_)(mib)$/i, 'MiB'],
  [/(^|_)(bytes?)$/i, 'bytes'], [/(^|_)(mhz)$/i, 'MHz'], [/(^|_)(ghz)$/i, 'GHz'], [/(^|_)(gbps)$/i, 'Gb/s'], [/(^|_)(mbps)$/i, 'Mb/s'],
];

export function unitFor(name) {
  for (const [re, u] of UNITS) if (re.test(name)) return u;
  if (/coefficient_of_variation|^cv$/i.test(name)) return 'ratio';
  return '';
}

const WORDS = { lr: 'Learning rate', acc: 'Accuracy', ppl: 'Perplexity', tps: 'Tokens/s', ttft: 'Time to first token', tpot: 'Time per output token', util: 'Utilisation', mem: 'Memory', temp: 'Temperature', cv: 'Variability (CV)', rel_l2: 'Relative L2 error', p50: 'p50', p90: 'p90', p95: 'p95', p99: 'p99' };

export function labelFor(name) {
  let n = String(name);
  const unitOnly = unitFor(n);
  n = n.replace(/(^|_)(tflops?|gflops?|tokens?_(per_)?s(ec)?|tok_s|tps|ms|us|secs?|seconds|elapsed|pct|percent|percentage|w|watts?|c|celsius|kb|mb|gb|gib|mib|bytes?|mhz|ghz|gbps|mbps)$/i, '');
  n = n.replace(/^gpu_/, 'GPU ').replace(/^cpu_/, 'CPU ').replace(/^mem_/, 'memory ').replace(/_utc$/, '').replace(/_id$/, '');
  n = n.replace(/coefficient_of_variation/, 'variability (CV)');
  n = n.trim().replace(/^_+|_+$/g, '');
  if (!n) return unitOnly || titleCase(name);
  if (WORDS[n.toLowerCase()]) return WORDS[n.toLowerCase()];
  const t = titleCase(n);
  return t.replace(/\bTflops\b/i, 'TFLOP/s').replace(/\bCv\b/, 'CV').replace(/\bGpu\b/, 'GPU').replace(/\bCpu\b/, 'CPU').replace(/\bKb\b/, 'KB').replace(/\bP(\d\d)\b/g, 'p$1');
}

// Large raw units are easier to read rescaled: KB → GB, bytes → MB/GB, ratio → %.
export function normalizeUnit(unit, values) {
  const v = values.filter((x) => x != null && Number.isFinite(x));
  if (!v.length) return { unit, factor: 1 };
  const max = Math.max(...v.map(Math.abs));
  if (unit === 'KB' && max >= 1e6) return { unit: 'GB', factor: 1 / 1048576 };
  if (unit === 'KB' && max >= 1e3) return { unit: 'MB', factor: 1 / 1024 };
  if (unit === 'MB' && max >= 1e4) return { unit: 'GB', factor: 1 / 1024 };
  if (unit === 'bytes' && max >= 1e9) return { unit: 'GB', factor: 1 / 1073741824 };
  if (unit === 'bytes' && max >= 1e6) return { unit: 'MB', factor: 1 / 1048576 };
  if (unit === 'ratio' && max <= 2) return { unit: '%', factor: 100 };
  if (unit === 'µs' && max >= 1e4) return { unit: 'ms', factor: 1 / 1000 };
  if (unit === 'ms' && max >= 1e5) return { unit: 's', factor: 1 / 1000 };
  return { unit, factor: 1 };
}
const scale = (arr, f) => (f === 1 ? arr : arr.map((x) => (x == null ? null : x * f)));

function familyKey(fileRel) {
  const base = path.basename(fileRel).replace(/\.csv$/i, '');
  const stripped = base.replace(/[-_ ]?run[-_ ]?\d+/i, '').replace(/[-_]?\d{8}T\d{6}(\.\d+)?Z?/g, '');
  return { dir: path.dirname(fileRel), key: stripped.replace(/[-_]+$/, ''), base };
}

function seriesLabel(fileRel, familyBase) {
  const base = path.basename(fileRel).replace(/\.csv$/i, '');
  const m = /run[-_ ]?(\d+)/i.exec(base);
  if (m) return `Run ${Number(m[1])}`;
  const rest = base.replace(familyBase, '').replace(/^[-_ ]+|[-_ ]+$/g, '');
  return rest || base;
}

function commonPrefix(strings) {
  if (!strings.length) return '';
  let p = strings[0];
  for (const s of strings) { while (!s.startsWith(p)) p = p.slice(0, -1); if (!p) break; }
  return p;
}

const MAX_POINTS = 1500;
function thin(values) {
  if (values.length <= MAX_POINTS) return values;
  const stride = Math.ceil(values.length / MAX_POINTS);
  return values.filter((_, i) => i % stride === 0);
}

/**
 * @param {{rel:string, text:string}[]} csvFiles  CSV files of one results session (rel = path inside the week folder)
 * @returns {{charts: object[], tables: object[]}}
 */
export function inferCharts(csvFiles) {
  const parsed = csvFiles.map((f) => {
    const { header, rows } = parseCSV(f.text);
    return { ...f, header, rows, cols: analyzeColumns(header, rows), ...familyKey(f.rel) };
  }).filter((f) => f.header.length && f.rows.length);

  // Group files that share a family key and an identical header → series of one chart.
  const families = new Map();
  for (const f of parsed) {
    const sig = `${f.dir}|${f.key}|${f.header.join(',')}`;
    if (!families.has(sig)) families.set(sig, []);
    families.get(sig).push(f);
  }

  const charts = [];
  const tables = [];
  let n = 0;
  for (const files of families.values()) {
    files.sort((a, b) => a.rel.localeCompare(b.rel, undefined, { numeric: true }));
    const first = files[0];
    const cols = first.cols;
    const rowsCount = Math.max(...files.map((f) => f.rows.length));
    const familyTitle = labelFor(first.key || first.base);

    // Always offer a table view for each file.
    for (const f of files) tables.push({ rel: f.rel, header: f.header, rows: f.rows.slice(0, 200), total: f.rows.length });

    if (rowsCount < 2) continue;

    // Pick X.
    let x = cols.find((c) => X_NAMES.includes(c.name.toLowerCase()) && (c.type === 'int' || c.type === 'number' || c.type === 'time') && !c.constant)
      || cols.find((c) => c.type === 'time' && c.monotonic)
      || cols.find((c) => (c.type === 'int') && c.monotonic && c.index === 0);
    const categorical = !x && cols[0] && (cols[0].type === 'string' || cols[0].type === 'time') && rowsCount <= 24 && files.length === 1;
    if (!x && !categorical) {
      // Fall back to the row index when there are enough rows and at least one numeric column.
      if (rowsCount >= 3 && cols.some((c) => (c.type === 'number' || c.type === 'int') && !c.constant)) {
        x = { name: 'index', type: 'int', values: first.rows.map((_, i) => i + 1), synthetic: true };
      } else continue;
    }

    const yCols = cols.filter((c) => (c.type === 'number' || c.type === 'int') && !c.constant && c !== x
      && !EXCLUDE_Y.includes(c.name.toLowerCase()) && !X_NAMES.includes(c.name.toLowerCase()));
    if (!yCols.length) continue;
    yCols.sort((a, b) => prio(a.name) - prio(b.name));

    if (categorical) {
      const labelsRaw = cols[0].values.map(String);
      const prefix = commonPrefix(labelsRaw);
      const labels = prefix.length >= 6 && labelsRaw.every((l) => l.length > prefix.length) ? labelsRaw.map((l) => l.slice(prefix.length)) : labelsRaw;
      // Group columns that share a unit suffix into one grouped-bar chart.
      const groups = new Map();
      for (const c of yCols) {
        const u = unitFor(c.name) || `__${c.name}`;
        if (!groups.has(u)) groups.set(u, []);
        groups.get(u).push(c);
      }
      for (const [u, gcols] of groups) {
        const rawUnit = u.startsWith('__') ? unitFor(gcols[0].name) : u;
        const { unit, factor } = normalizeUnit(rawUnit, gcols.flatMap((c) => c.values));
        const series = gcols.slice(0, 6).map((c) => ({ name: labelFor(c.name), values: scale(c.values, factor) }));
        const xLabel = labelFor(cols[0].name);
        charts.push({
          id: `chart-${++n}`, kind: 'bar', file: first.rel, files: files.map((f) => f.rel), priority: Math.min(...gcols.map((c) => prio(c.name))), points: labels.length,
          title: gcols.length > 1 ? `${unit || 'Values'} by ${xLabel.toLowerCase()} — ${series.map((s) => s.name.toLowerCase()).join(', ')}` : `${series[0].name} by ${xLabel.toLowerCase()}`,
          unit, xLabel, x: { name: cols[0].name, type: 'category', values: labels }, series,
        });
      }
      continue;
    }

    for (const yc of yCols.slice(0, 4)) {
      const rawUnit = unitFor(yc.name);
      const { unit, factor } = normalizeUnit(rawUnit, files.flatMap((f) => f.cols.find((c) => c.name === yc.name)?.values || []));
      const series = files.slice(0, 6).map((f) => {
        const col = f.cols.find((c) => c.name === yc.name);
        const xs = x.synthetic ? f.rows.map((_, i) => i + 1) : f.cols.find((c) => c.name === x.name).values;
        const pts = xs.map((xv, i) => [xv, col.values[i] == null ? null : col.values[i] * factor]).filter((p) => p[0] != null && p[1] != null);
        return { name: files.length > 1 ? seriesLabel(f.rel, first.key) : labelFor(yc.name), points: thin(pts) };
      });
      const stats = series.map((s) => ({ name: s.name, median: median(s.points.map((p) => p[1])), min: Math.min(...s.points.map((p) => p[1])), max: Math.max(...s.points.map((p) => p[1])) }));
      charts.push({
        id: `chart-${++n}`, kind: 'line', file: first.rel, files: files.map((f) => f.rel), priority: prio(yc.name), points: Math.max(...series.map((s) => s.points.length)),
        title: `${labelFor(yc.name)} by ${x.synthetic ? 'row' : labelFor(x.name).toLowerCase()}${files.length > 1 ? ` — ${familyTitle}` : ''}`,
        unit, xLabel: x.synthetic ? 'Row' : labelFor(x.name), xType: x.type === 'time' ? 'time' : 'number',
        series, stats, extra: files.length > 6 ? files.length - 6 : 0,
      });
    }
  }
  // Most informative first: headline metrics (throughput, tokens/s, latency…) before housekeeping telemetry, then richer charts.
  charts.sort((a, b) => a.priority - b.priority || b.points - a.points);
  charts.forEach((c, i) => { c.id = `chart-${i + 1}`; });
  return { charts, tables };
}

function prio(name) {
  const n = name.toLowerCase();
  const i = PRIORITY.findIndex((p) => n.includes(p));
  return i === -1 ? 99 : i;
}

// A compact set of headline numbers from a runs-style CSV (one row per run).
export function keyStatsFromRuns(header, rows) {
  const cols = analyzeColumns(header, rows);
  const stats = [];
  if (rows.length) stats.push({ label: rows.length === 1 ? 'run' : 'runs', value: String(rows.length) });
  const med = cols.find((c) => /^median_/i.test(c.name) && (c.type === 'number' || c.type === 'int'));
  if (med) {
    const m = median(med.values.filter((v) => v != null));
    const { unit, factor } = normalizeUnit(unitFor(med.name), med.values);
    const what = labelFor(med.name).replace(/^Median\s*/i, '').trim().toLowerCase();
    if (Number.isFinite(m)) stats.push({ label: what ? `median ${what} across runs` : 'median across runs', value: m * factor, numeric: true, unit });
  }
  for (const c of cols) {
    if (c.type === 'bool' && /pass|ok|success|correct|stab/i.test(c.name)) {
      const vals = c.values.map((v) => String(v).toLowerCase());
      const passed = vals.filter((v) => ['true', 'yes', 'pass', 'passed'].includes(v)).length;
      const label = labelFor(c.name).replace(/ Pass$/i, '').toLowerCase();
      // A "stability" flag is advisory in this pipeline: a failed check is a warning, not a failure.
      const advisory = /stab/i.test(c.name);
      const value = passed === vals.length ? 'PASS' : advisory && passed === 0 ? 'WARNING' : `${passed}/${vals.length}`;
      stats.push({ label, value, status: passed === vals.length ? 'good' : 'warn' });
    }
  }
  return stats.slice(0, 4);
}
